...
$().ready(function() {	
		...
		//	content_css : "comments/css/content.css",
...
$(document).ready(function(e){
	$(".delComment").live("click", function() {
		...
		var dataString = 'idd=' + getvalue + '&passport=' + passport + '&eventComments=del';
		$.ajax({
			type: "POST",
			url: "",
			data: dataString,
	...
	$(".replyComment").live("click", function() {
		...
		$.ajax({
			...
			$('#RformComment textarea.tinymce').tinymce({
				// Location of TinyMCE script
				script_url : '/comments/js/tiny_mce/tiny_mce.js',
				relative_urls : false,
				// General options
				theme : "advanced",language:"ru",
	...
	var dataString = 'loginComment=' + loginComment + '&addComment=' + addComment + '&personaComment=' 
			...
			+ '&emailComment=' + email + '&textComment=' + encodeURIComponent(comment) + '&anon=' + anon + '&posturlComment=' 
	...
	{
	$("#ajaxComment").show();
	$("#ajaxComment").fadeIn(400).html('<img src="/comments/images/comment/ajax-bar.gif" align="absmiddle">&nbsp;<span class="loading"></span>');
	$.ajax({
		type: "POST",
		url: "",
		data: dataString,
